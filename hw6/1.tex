...
\\
And we never transtion to final state if we dont find \(ccd\) in the input string because we can only transition to final state from \(q_{shiftingdone}\) state which
we can only reach if we found \(ccd\) and replaced it with \(abd\).
